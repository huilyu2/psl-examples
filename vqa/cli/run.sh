...
function runEvaluation() {
   ...
   java -jar "${JAR_PATH}" -infer -model "${BASE_NAME}.psl" -data "${BASE_NAME}-eval.data" -output inferred-predicates ${ADDITIONAL_EVAL_OPTIONS} ${ADDITIONAL_PSL_OPTIONS}
   if [[ "$?" -ne 0 ]]; then
